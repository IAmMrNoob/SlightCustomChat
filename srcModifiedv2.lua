...
local HiddenPropertyChangedSignal = loadstring(game:HttpGet("https://raw.githubusercontent.com/IAmMrNoob/RFE/refs/heads/main/Misc/fakePropertyChangeEvent.lua", true))()
local method,universalChanged = 2,true
...
pcall(function()
	setscriptable(lp, "CloudEditCameraCoordinateFrame", true)
    local discont = lp:GetPropertyChangedSignal("CloudEditCameraCoordinateFrame"):Connect(function() -- recently discovered this that it would error if an executor doesnt make setscriptable properly
        
    end)
    discont:Disconnect()
    universalChanged = false -- setting it to 1 which is not uni
end)
-- doing tings main
...
function fml.tnum(str,new)
    ...
    local maxCHARACTERS = 6 --- i lied everythime i said about being able to do 12 characters now im gonna try 8 to see if 24 numbers is possible instead
    ...
    if new then
        for i = 1, tLength, 1 do
            ...
            table.insert(returnValue,finishedSet)
...
function fml.tstr(tn)
    ...
    local returnValue = ""
    local maxCHARACTERS = 3

    for _, data in pairs(tn) do
        data = tostring(data)
        for i = 1, string.len(data)/maxCHARACTERS,1 do
            local current = string.sub(data,1,maxCHARACTERS)
            pcall(function()
...
local function catch(plr,cf)
    ...
    warn("catch1",X,Y,Z,rX,rY,rZ)
    if rX ~= nil then -- basicly since i set them nil ik when or when not to switch yk
        local channel = mod.Listeners[math.round(rZ)] -- rotation on the Zaxis being the channel rotations only can go from -360 to 360
        ...
        warn("catch2raw",X,Y,Z,math.round(rX),math.round(rY),math.round(rZ))
        if mod.Listeners[math.round(rZ)] ~= nil then---- check if it exist
            ...
            for i, Callbacks in pairs(channel) do
                Callbacks(plr,X,Y,Z,math.round(rX),math.round(rY))--- i have to use whole numbers since rotations always come  or sometimes come in decimeals
            end
        end
    else
...
do -- setup kinda
    function onJoin(plr)
        ...
        if universalChanged then
            HiddenPropertyChangedSignal(plr,"CloudEditCameraCoordinateFrame"):Connect(function()
                catch(plr,cloudCF.get(plr))
            end)
        else
            plr:GetPropertyChangedSignal("CloudEditCameraCoordinateFrame"):Connect(function()
                catch(plr,cloudCF.get(plr))
            end)
        end
    end 
...
function SendRecieveChannels.Create(id,noorderthing) -- id in a way as im gonna be trying to make a channel for sending and recieving info like a queue in a way
    --- if no order thing is set to true then you dont need to enter order channel or whatever for data
    ...
    local ChannelId = id + 100
    local SRchannel = mod.Listen(ChannelId)-- my little starting point ig
    SendRecieveChannels.InUse[id] = false
    --- start 558
    --- end 311
    --- info 428
    local start,info,end_ = 5,4,3
    local dataset = {}
    local testtime = {}
    SRchannel.addListener(function(plr,data1,data2,data3,a,b)
        --- a = order sometimes tho
        --- b was planed for how many total packets there were but then after i thought about what if the thing sent over passes 360 packets so then i just scrapped it
        if a == start and b == info then-- we reset and start
            table.clear(dataset)
            table.clear(testtime)
testtime[1] = DateTime.now().UnixTimestampMillis
        ...
        elseif a == info and b == end_ then -- we end and call our main function that wants to use the data
            local answer = fml.tstr(dataset)
testtime[2] = DateTime.now().UnixTimestampMillis
            print("dataset2",dataset,#dataset,dataset[1],answer)
game:GetService("TestService"):Message(tostring((testtime[2]-testtime[1])/1000).."s to complete receiving packets = "..#dataset)
            SendRecieveChannels.Callback(plr,answer)
            --warn("closed channel for ".. tostring(id))
        elseif noorderthing then--- this means we are now gonna start js captureing everything but without the order thing so it would then mean you could possible send long datas
            if data1 < 100 then
                data1 = ""
            end
            if data2 < 100 then
            ...
            warn("dataset1",dataset,#dataset,dataset[1])
            table.insert(dataset,tostring(data1)..tostring(data2)..tostring(data3)) -- for now im gonna make it so b is the order of it but later on i will be using both as data or more
            ...
            warn("dataset1",dataset,#dataset,dataset[1])
            dataset[a] = tostring(data1)..tostring(data2)..tostring(data3) -- for now im gonna make it so b is the order of it but later on i will be using both as data or more
            --warn("Recieved for ".. tostring(id) .. " " .. tostring(b) .. " data: " .. tostring(a))
        end
    ...
    function returnValue.setStart()
        ...
        SRchannel.Putraw(mod.infoWr.toCF(0,0,0,start,info,ChannelId))
    end
    function returnValue.setEnd()
        --warn(info,end_)
        SendRecieveChannels.InUse[id] = false
        SRchannel.Putraw(mod.infoWr.toCF(0,0,0,info,end_,ChannelId))
    end
    function returnValue.Put(data1,data2,data3,order)
        --warn(a,b)
        print(data1,data2,data3,order,0,ChannelId,mod.infoWr.toCF(data1,data2,data3,order,0,ChannelId))
        if noorderthing then
            SRchannel.Putraw(mod.infoWr.toCF(data1,data2,data3,-1,0,ChannelId))
        else
            SRchannel.Putraw(mod.infoWr.toCF(data1,data2,data3,order,0,ChannelId))
        end
    end
...
function SendRecieveChannels.send(str,progressCallback)
    ...
    if UsableChannelID ~= nil then
        task.spawn(function()
            local dataset = fml.tnum(str,true)
            --[[
            ...
            for order, data in pairs(dataset) do
                ...
                local d1,d2,d3 = tostring(data):sub(0,6),tostring(data):sub(7,12),tostring(data):sub(13,18)
                warn("send1",data)
                if d1 == "" then --- not possible but like maybe you never know
                    d1 = 0
                end
                if d2 == "" then
                    d2 = 0
                ...
                warn("send2",d1,d2,d3,order)
                SendRecieveChannels.srC[UsableChannelID].Put(d1,d2,d3,order)
                task.wait(wT)
            end
            task.wait(wT)
